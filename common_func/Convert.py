...
def baseline_qry(frm_data):
    ...
    if 'calibrated_baseline' in frm_data:
        baseline['calibrated_baseline'] = (
            Pillar_Survey.objects.get(pk = frm_data['calibrated_baseline'].pk))
        baseline['site'] = baseline['calibrated_baseline'].pillar_survey.baseline
    
        sd_m = Std_Deviation_Matrix.objects.select_related().filter(
                                calibrated_baseline__pk = 
                                baseline['calibrated_baseline'].pk)
        baseline['std_dev_matrix'] = ({s.from_pillar.name + ' - ' + s.to_pillar.name:
                                        model_to_dict(s) for s in sd_m})

        cd = Certified_Distance.objects.select_related().filter(
                                calibrated_baseline__pk = 
                                baseline['calibrated_baseline'].pk)
        baseline['certifed_dist'] ={d.to_pillar.name:model_to_dict(d) for d in cd}
